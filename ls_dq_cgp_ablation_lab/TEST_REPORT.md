...
本轮目标范围内的完整模型、反事实、四个 inference-only 消融、五个重训练消融均已完成 Test。本次重新运行了本地的 Moment-DETR-GMR baseline 与 DQ-CGPv3 发布 checkpoint；两者均与归档指标一致，并在新的输出目录生成了 1,036 条 prediction JSONL，完成独立核验。
...
| Moment-DETR-GMR Baseline | 本次重测完成 | `outputs/moment_detr_gmr_baseline_seed2023/test_recheck/` |
| 早期 Native Binding | 已完成总体 Test | `results/component_attribution_seed2023/native_binding/test/` |
...
## 本次 Moment-DETR-GMR Baseline 重测

命令：

```bash
/home/guoxiangyu/miniconda3/envs/GMR/bin/python \
dq_cgp_working_part_lab/evaluate_variant.py \
  --checkpoint /home/guoxiangyu/VLMbasedIter_momentretrival/DQ-CGP-main/outputs/dq_cgp_working_part_seed2023/baseline/best.ckpt \
  --output outputs/moment_detr_gmr_baseline_seed2023/test_recheck \
  --split test --device cuda
```

检查结果：预测与 GT 的共享 qid 为 1,036，缺失 qid 为 0。该 checkpoint 为 Epoch 11；mAP 6.14、mR@1/3/5 = 4.16/6.48/7.89、mIoU@1/3/5 = 12.30/11.65/11.61、AUROC 71.87、G-mIoU@1/3/5 = 5.67/2.44/1.49，与归档 baseline Test 指标完全一致。
...
| 分桶 | 样本数 | Full mAP | DQ-CGPv3 mAP | Baseline mAP | `Full - DQ` | `Full - Baseline` |
|---|---:|---:|---:|---:|---:|---:|
| SportsMoments ≤3 秒 | 81 | 3.71 | 4.14 | 0.74 | -0.43 | +2.97 |
| SportsMoments 4--5 秒 | 72 | 26.30 | 26.79 | 10.14 | -0.49 | +16.16 |
| SportsMoments ≥6 秒 | 63 | 32.33 | 20.93 | 11.15 | **+11.40** | **+21.18** |
| 单目标 | 384 | 18.27 | 15.30 | 6.17 | +2.97 | +12.10 |
| 多目标 | 160 | 17.58 | 16.02 | 6.06 | +1.56 | +11.52 |
| Early | 96 | 18.32 | 13.86 | 4.49 | +4.46 | +13.83 |
| Middle | 357 | 18.53 | 16.62 | 7.06 | +1.91 | +11.47 |
| Late | 91 | 15.98 | 12.88 | 4.25 | +3.10 | +11.73 |

相对 Moment-DETR-GMR Baseline，Full 在所有主桶上均有明显优势，尤其是 SportsMoments 的 ≥6 秒目标（+21.18 mAP）和所有时间位置。相对 DQ-CGPv3 发布 checkpoint，Full 的主要优势集中于 SportsMoments 的 ≥6 秒目标，以及 Early/Middle/Late 各时间位置；Full 对 ≤5 秒目标没有优势。由于 SportsMoments 的长度与数据源、单目标属性相关，长度结论仅限该数据源内部。
