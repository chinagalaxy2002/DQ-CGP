"""Late-Semantic DQ-CGP with Query-conditioned Attentive Prompt Pooling.
...
2. BPS: Synthesizes dynamic prompt tokens from shared learnable basis prompts.
3. QAP: Pools prompt tokens using candidate and sentence context.
4. FRF: Generates adapted query-specific semantic feature E_adapt^q.
5. Semantic Matcher: Scores cosine similarity between D2 query state h_q and E_adapt^q.
"""
...
class LSDQCGPOutput(NamedTuple):
    ...
    prompt_sequence: Tensor            # [B, Q, prompt_length, D]
    prompt_attention: Tensor           # [B, Q, prompt_length]
...
class LateSemanticCGP(nn.Module):
    ...
    def __init__(
        ...
        # QAP: [V_q; E_static] attends over synthesized prompt positions.
        self.prompt_query_proj = nn.Linear(2 * hidden_dim, hidden_dim)
        self.prompt_key_proj = nn.Linear(hidden_dim, hidden_dim, bias=False)
        self.prompt_value_proj = nn.Linear(hidden_dim, hidden_dim, bias=False)
    ...
    def forward(
        ...
        # 2. BPS: Synthesize multiple prompt tokens.
        ...
        # 2.5 QAP: Candidate-conditioned attentive prompt pooling.
        prompt_condition = torch.cat([v_detached, e_static_expanded], dim=-1)  # [B, Q, 2D]
        prompt_query = self.prompt_query_proj(prompt_condition)                # [B, Q, D]
        prompt_keys = self.prompt_key_proj(prompt_seq)                         # [B, Q, P, D]
        prompt_values = self.prompt_value_proj(prompt_seq)                     # [B, Q, P, D]
        ...
        pooled_prompt = self.basis_norm(pooled_prompt)
