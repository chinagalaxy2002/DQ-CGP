...
class StartEndDataset(Dataset):
    ...
    def __init__(
        ...
        return_query_semantic_mask=False,
    ):
        ...
        self.use_query_attention_mask = bool(use_query_attention_mask)
        self.return_query_semantic_mask = bool(return_query_semantic_mask)
    ...
    def __getitem__(self, index):
        ...
        query_feat, query_semantic_mask = self._get_query_data_by_qid(meta["qid"])
        model_inputs = {"query_feat": query_feat}
        if query_semantic_mask is not None:
            model_inputs["query_semantic_mask"] = query_semantic_mask
    ...
    def _get_query_feat_by_qid(self, qid):
        query_feat, _ = self._get_query_data_by_qid(qid)
        return query_feat

    def _get_query_data_by_qid(self, qid):
        q_feat_path = join(self.q_feat_dir, f"qid{qid}.npz")
        with np.load(q_feat_path) as q_data:
            q_feat = q_data[self.q_feat_type].astype(np.float32)
            q_mask = None
            if self.use_query_attention_mask or self.return_query_semantic_mask:
                if "attention_mask" not in q_data:
                    raise KeyError(
                        "A query attention mask was requested but attention_mask is missing: "
                        f"{q_feat_path}"
                    )
                q_mask = q_data["attention_mask"].astype(bool)

        if self.q_feat_type == "last_hidden_state":
            q_feat = q_feat[:self.max_q_l]
            if q_mask is not None:
                q_mask = q_mask[:self.max_q_l]
                if len(q_mask) != len(q_feat):
                    raise ValueError(
                        f"Text feature/mask length mismatch in {q_feat_path}: "
                        f"{len(q_feat)} vs {len(q_mask)}"
                    )
                if self.use_query_attention_mask:
                    q_feat = q_feat[q_mask]
                    q_mask = np.ones(len(q_feat), dtype=bool)
                    if len(q_feat) == 0:
                        raise ValueError(f"No valid query tokens in {q_feat_path}")
        q_feat = l2_normalize_np_array(q_feat)
        semantic_mask = None
        if self.return_query_semantic_mask:
            if q_mask is None:
                q_mask = np.ones(len(q_feat), dtype=bool)
            semantic_mask = torch.from_numpy(q_mask.astype(np.float32))
        return torch.from_numpy(q_feat), semantic_mask
...
def prepare_batch_inputs(batched_model_inputs, device, non_blocking=False):
    ...
    if "query_semantic_mask" in batched_model_inputs:
        model_inputs["src_txt_semantic_mask"] = batched_model_inputs[
            "query_semantic_mask"
        ][0].to(device, non_blocking=non_blocking)
