"""Evaluate Encoder-Text LS-DQ-CGP and its counterfactual modes."""
...
from ls_dq_cgp_encoder_text_lab.ls_dq_cgp_model import LSDQCGPModel
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate Encoder-Text LS-DQ-CGP")
    ...
    counterfactual.add_argument(
        ...
        action="store_true",
        help="Use pre-encoder E_static instead of encoder E_enc for both RCG and FRF",
    )
    ...
    if args.context_roll:
        mode_name = "context_roll"
    elif args.pre_encoder_condition:
        mode_name = "pre_encoder_condition"
    else:
        mode_name = "active"
    ...
    model.pre_encoder_condition = args.pre_encoder_condition
    ...
    result_record = {
        "variant": (
            "encoder_text_ls_dq_cgp_exist"
            if has_exist_head
            else "encoder_text_ls_dq_cgp"
        ),
    ...
    print("\n" + "=" * 60)
    print(f"Encoder-Text LS-DQ-CGP Evaluation Summary [{args.split.upper()}] (Mode: {mode_name}, Exist Head: {has_exist_head})")
    print("=" * 60)
