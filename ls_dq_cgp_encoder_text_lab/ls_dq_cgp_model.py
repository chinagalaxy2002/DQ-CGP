"""Encoder-Text LS-DQ-CGP model wrapper and loss integration."""
...
from ls_dq_cgp_encoder_text_lab.cgp_module import (
    EncoderTextLSDQCGPOutput,
    EncoderTextLateSemanticCGP,
)
...
class LSDQCGPModel(nn.Module):
    """Equip Moment-DETR with encoder-conditioned late semantic adaptation."""
    ...
    def __init__(
        ...
        self.cgp = EncoderTextLateSemanticCGP(
        ...
        self.pre_encoder_condition = False
    ...
    def forward(
        ...
        if bool((semantic_count == 0).any()):
            raise ValueError("No valid CLIP text tokens for semantic pooling")
        ...
        encoder_weights = semantic_mask.to(txt_mem.dtype).unsqueeze(-1)
        encoder_semantic = (
            (txt_mem * encoder_weights).sum(dim=1)
            / semantic_count.to(txt_mem.dtype)
        )                                                         # [B, d]
        ...
        cgp_out: EncoderTextLSDQCGPOutput = self.cgp(
            visual_context=v_context,
            static_semantic=static_semantic,
            encoder_semantic=encoder_semantic,
            query_states=d2_queries,
            pre_encoder_condition=self.pre_encoder_condition,
        )
        ...
        out = {
            ...
            "static_semantic": static_semantic,
            "encoder_semantic": encoder_semantic,
            "condition_semantic": cgp_out.condition_semantic,
        }
