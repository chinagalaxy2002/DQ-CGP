"""Training script for Token-Selective Late-Semantic DQ-CGP."""
...
from ls_dq_cgp_token_lab.ls_dq_cgp_model import LSDQCGPModel, install_ls_dq_cgp_loss

logger = logging.getLogger(__name__)


def main():
    parser = argparse.ArgumentParser(description="Train Token LS-DQ-CGP on Soccer-GMR")
    parser.add_argument("--output", default="outputs/token_ls_dq_cgp_seed2023", help="Output directory")
    ...
    opt.component_variant = (
        "token_ls_dq_cgp_exist" if args.use_exist_head else "token_ls_dq_cgp"
    )

    experiment_meta = {
        "variant": opt.component_variant,
        "seed": args.seed,
        "model": "TokenSelectiveLateSemantic_MomentDETR",
    ...
    logger.info(f"Starting Token LS-DQ-CGP training with exist_head={args.use_exist_head}, semantic_mask={opt.query_cgp_use_semantic_mask}, seed={args.seed}, total epochs={args.epochs}...")
