...
from ls_dq_cgp_token_lab.cgp_module import (
    TokenLSDQCGPOutput,
    TokenSelectiveLateSemanticCGP,
)
...
class LSDQCGPModel(nn.Module):
    ...
    def __init__(
        ...
        self.cgp = TokenSelectiveLateSemanticCGP(
    ...
    def forward(
        ...
        if bool((semantic_count == 0).any()):
            raise ValueError("No valid CLIP text tokens for semantic selection")
        ...
        for batch_index in range(token_select_mask.shape[0]):
            valid_indices = torch.nonzero(
                token_select_mask[batch_index], as_tuple=False
            ).flatten()
        ...
        out = {
            ...
            "token_attention": cgp_out.token_attention,
        }
